...
from models.load_data import load_training_data
    ...
    with strategy.scope():
        ...
        if args.train:
            ...
            x, y = load_training_data(
                "../../Amharic-English-Machine-Translation-Corpus/test-am.txt",
                "../../Amharic-English-Machine-Translation-Corpus/test-en.txt",
                INPUT_WIDTH, OUTPUT_WIDTH, BATCH_SIZE)
            csv_logger = CSVLogger('{}_log.csv'.format(model_name), append=True,
                                   separator='\t')
            model.fit(x, y, batch_size=BATCH_SIZE, epochs=args.train,
                      verbose=True, callbacks=[csv_logger])
